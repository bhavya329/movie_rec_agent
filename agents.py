...
def build_email_html(items, feedback):
    ...
    has_feedback_url = bool(os.environ.get("FEEDBACK_BASE_URL",""))
    cards = ""

    for i, m in enumerate(items):
        border    = "border:1.5px solid #185FA5;" if i==0 else "border:1px solid #e0e0e0;"
        top_badge = ('<span style="background:#E6F1FB;color:#0C447C;font-size:11px;'
                     'padding:2px 8px;border-radius:99px;margin-bottom:8px;display:inline-block;">'
                     '⭐ Top pick</span><br>') if i==0 else ""
        type_badge = ('<span style="background:#E1F5EE;color:#085041;font-size:11px;'
        ...
        if has_feedback_url:
            ...
            def fb_url(thumb):
                p = urllib.parse.urlencode({
                    "id":str(m["id"]), "thumb":thumb,
                    "title":m["title"][:60],
                    "genres":",".join(m.get("genres",[])[:3]),
                    "lang":m.get("language",""),
                    "media":m["media_type"],
                })
                return f"{os.environ['FEEDBACK_BASE_URL']}?{p}"

            fb_html = (
                f'<div style="margin-top:10px;display:flex;gap:8px;">'
                f'<a href="{fb_url("up")}" style="text-decoration:none;background:#EAF3DE;'
                f'color:#27500A;padding:5px 14px;border-radius:99px;font-size:12px;">👍 Liked it</a>'
                f'<a href="{fb_url("down")}" style="text-decoration:none;background:#FCEBEB;'
                f'color:#791F1F;padding:5px 14px;border-radius:99px;font-size:12px;">👎 Not for me</a>'
                f'</div>'
            )
...
<div style="background:#fff;{border}border-radius:12px;padding:16px;margin-bottom:12px;">
  ...
  {fb_html}
    ...
    fb_note  = ('<p style="font-size:12px;color:#666;background:#F5F5F5;padding:10px 14px;'
                'border-radius:8px;margin-top:4px;">👍 / 👎 buttons above help me learn your taste — '
                'your feedback is remembered every week.</p>') if has_feedback_url else ""
...
<div style="max-width:560px;margin:0 auto;">
  ...
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:12px;padding:16px 20px;margin-bottom:16px;font-size:14px;color:#444;line-height:1.6;">
    <strong>{movies_c} movie{'s' if movies_c!=1 else ''} + {series_c} series</strong> available on Indian OTT this week,
...
# ---------------------------------------------------------------------------
# Tool 9 — Send to multiple recipients
# ---------------------------------------------------------------------------
